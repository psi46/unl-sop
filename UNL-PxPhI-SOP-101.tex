...
\begin{enumerate}
    ...
    \item Record the serial numbers of the HDI in the Purdue database (``MAIN MENU''$\rightarrow$``HDI Submit''). Leave ``Additional Notes'' empty if nothing special to report, otherwise report any findings.
    \item Perform the visual inspection of HDI according to SOP~201.
    \item Update Purdue database. The following step needs to be done for every HDI in the delivery:
    \begin{enumerate}
        \item Retrieve the HDI: ``MAIN MENU''$\rightarrow$``Part List'', click on HDI on the list
